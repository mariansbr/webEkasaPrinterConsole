...
{$IFDEF SUPERTIMEZONE}
{ +. }
function JavaToDelphiDateTime(const dt: Int64; const TimeZone: SOString = '')
  : TDateTime;
function DelphiToJavaDateTime(const dt: TDateTime;
  const TimeZone: SOString = ''): Int64;
...
function ISO8601DateToJavaDateTime(const str: SOString; var ms: Int64;
  const TimeZone: SOString = ''): Boolean;
...
function DelphiDateToISO8601(const ADate: TDateTime;
  AInputIsUTC: Boolean = true): SOString;
function TryISO8601ToDelphiDate(const AISODate: SOString; out Value: TDateTime;
  AReturnUTC: Boolean = true): Boolean;

function DelphiDateTimeToUnix(const AValue: TDateTime;
  AInputIsUTC: Boolean = true): Int64;
function UnixToDelphiDateTime(const AValue: Int64; AReturnUTC: Boolean)
  : TDateTime;
...
begin
  ...
  ms := dt mod 1000;
  p := gmtime(@l);
  Result := EncodeDateTime(p^.tm_year + 1900, p^.tm_mon + 1, p^.tm_mday,
...
function TzSpecificLocalTimeToSystemTime(const lpTimeZoneInformation
  : PTimeZoneInformation; const lpLocalTime: PSystemTime;
  lpUniversalTime: PSystemTime): BOOL;
var
  ft: TFileTime;
  lBias: LongInt;
  t: Int64;
...
begin
  ...
  if (st.hour >= 24) or (st.minute >= 60) or (st.second >= 60) or
    (st.ms >= 1000) or (st.week > 53) then
    goto error;

  if not havetz then
    st.Bias := GetTimeBias;

  ms := st.ms + st.second * 1000 + (st.minute + st.Bias) * 60000 + st.hour
    * 3600000;
  if havedate then
  begin
    ...
    begin
      ...
      for v := 1 to st.month - 1 do
        inc(ms, Int64(DayTable^[v]) * Int64(86400000));
      // FPC: Fix Overflow Error (EIntOverflowError: Arithmertic overflow)
    end;
    dec(st.year);
...
{$IFEND}
    ...
    begin
      ...
      Result := Format(SOffsetFormat, [Result, Neg[Bias > 0],
...
{ -$DEFINE USE_ISO8601ToDate }
// optional: DateItils.pas: TryISO8601ToDate. !! Slowly through the use of try-except !!
{$IFEND}

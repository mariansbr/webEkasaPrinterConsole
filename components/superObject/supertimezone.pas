...
// https://code.google.com/p/superobject/issues/detail?id=58
type
  ...
  TGetDynamicTimeZoneInformation = function(var pTimeZoneInformation
    : DYNAMIC_TIME_ZONE_INFORMATION): DWORD; stdcall;
...
{$IFDEF MSWINDOWS}
  private const
    TZ_TZI_KEY = '\SYSTEM\CurrentControlSet\Control\TimeZoneInformation';
    { Vista and + }
...
{$ENDIF}
  private
    ...
    // {$IFDEF MSWINDOWS}
    { Windows Internals }
    function TzSpecificLocalTimeToSystemTime(const lpTimeZoneInformation
      : pTimeZoneInformation; var lpLocalTime, lpUniversalTime
      : TSystemTime): BOOL;

    function SystemTimeToTzSpecificLocalTime(const lpTimeZoneInformation
  ...
  private
    ...
    class function GetSuperTimeZoneInstance(const Name: string)
      : TSuperTimeZone; static;
  ...
  public
    ...
    function ISO8601ToJava(const ISO8601Date: SOString;
      var JavaDateTime: Int64): Boolean;
    function ISO8601ToDelphi(const ISO8601Date: SOString;
      var DelphiDateTime: TDateTime): Boolean;

    { TZ Info }
    class function GetCurrentTimeZone: SOString;
    function GetTimeZoneInformation(Year: Word;
    ...
    class property Zone[const TimeZoneName: string]: TSuperTimeZone
      read GetSuperTimeZoneInstance;
  end;
...
{$WARN SYMBOL_PLATFORM OFF}
  ...
  { Windows 2000+ }
function _SystemTimeToTzSpecificLocalTime(lpTimeZoneInformation
  : pTimeZoneInformation; var lpUniversalTime, lpLocalTime: TSystemTime): BOOL;
  stdcall; external kernel32 name 'SystemTimeToTzSpecificLocalTime' delayed;

{ Windows XP+ }
function _TzSpecificLocalTimeToSystemTime(lpTimeZoneInformation
  : pTimeZoneInformation; var lpLocalTime, lpUniversalTime: TSystemTime): BOOL;
  stdcall; external kernel32 name 'TzSpecificLocalTimeToSystemTime' delayed;

(* EXtended version - DST Aware *)

{ Windows 7+ }
function _TzSpecificLocalTimeToSystemTimeEx(const lpTimeZoneInformation
  : PDynamicTimeZoneInformation; const lpLocalTime: PSystemTime;
  var lpUniversalTime: TSystemTime): BOOL; stdcall;
  external kernel32 name 'TzSpecificLocalTimeToSystemTimeEx' delayed;

{ Windows 7+ }
function _SystemTimeToTzSpecificLocalTimeEx(const lpTimeZoneInformation
  : PDynamicTimeZoneInformation; const lpUniversalTime: PSystemTime;
  var lpLocalTime: TSystemTime): BOOL; stdcall;
  external kernel32 name 'SystemTimeToTzSpecificLocalTimeEx' delayed;
...
begin
  ...
  Move(TimeZoneName[1], DTZI.TimeZoneKeyName, (Length(TimeZoneName) + 1) *
    SizeOf(SOChar));
...
begin
  ...
  { Replaces the TimeZoneKeyName member with specified TimeZoneName }
  Move(TimeZoneName[1], DTZI.TimeZoneKeyName[0], Length(TimeZoneName) *
    SizeOf(SOChar));
...
class function TSuperTimeZone.GetSuperTimeZoneInstance(const Name: string)
  : TSuperTimeZone;
begin
...
begin
  ...
  if GetTimeZoneInformation(local.wYear, TZI) and
...
begin
  ...
  if GetTimeZoneInformation(UTC.wYear, TZI) and SystemTimeToTzSpecificLocalTime
    (@TZI, UTC, local) then
...
function TSuperTimeZone.DelphiToISO8601(DelphiDateTime: TDateTime): SOString;
const
...
begin
  DateTimeToSystemTime(DelphiDateTime, local);
  iso := Format(ISO_Fmt, [local.wYear, local.wMonth, local.wDay, local.wHour,
...
var
  ...
  DayTable: PDayTable;
begin
  if ParseISO8601Date(ISO8601Date, st, dayofyear, week, Bias, havetz,
    havedate) then
  begin
    if (not havetz) and GetTimeZoneInformation(st.wYear, TZI) and
      TzSpecificLocalTimeToSystemTime(@TZI, st, UTC) then
      Bias := Trunc((SystemTimeToDateTime(st) - SystemTimeToDateTime(UTC)) *
        MinsPerDay);
    JavaDateTime := st.wMilliseconds + st.wSecond * 1000 + (st.wMinute + Bias) *
      60000 + st.wHour * 3600000;
    if havedate then
    begin
      ...
      Dec(st.wYear);
      Inc(JavaDateTime, Int64((st.wYear * 365) + (st.wYear div 4) -
        (st.wYear div 100) + (st.wYear div 400) + st.wDay + dayofyear - 719163)
        * 86400000);
    end;
    Result := True;
  end
    ...
    try
      ...
      begin
        ...
        begin
          CloseKey;
          RootKey := HKEY_USERS;
          if OpenKeyReadOnly('.DEFAULT\' + TZ_KEY) and
            ValueExists(TZ_KEYNAME) then
            Result := Trim(ReadString(TZ_KEYNAME))
          else
            Result := '';
        end;
    ...
    try
      ...
      if OpenKeyReadOnly(KeyName + '\Dynamic DST') then
        try
          ...
          Retry := False;
          repeat
            while (ChangeYear > 0) and
              (not ValueExists(IntToStr(ChangeYear))) do
            begin
              Dec(ChangeYear);
            ...
            begin
              ReadBinaryData(IntToStr(ChangeYear), RegTZI,
                SizeOf(TRegistryTZI));
              if RegTZI.DaylightChangeTime.wMonth >
                RegTZI.StandardChangeTime.wMonth then
              begin
                Dec(ChangeYear);
                Retry := not Retry;
              end;
...
begin
  ...
  begin
    (* if year is 0 then date is in day-of-week format, otherwise
      * it's absolute date.
    *)
    if ((pTZinfo^.StandardDate.wMonth = 0) or ((pTZinfo^.StandardDate.wYear = 0)
      and ((pTZinfo^.StandardDate.wDay < 1) or (pTZinfo^.StandardDate.wDay > 5)
      or (pTZinfo^.DaylightDate.wDay < 1) or (pTZinfo^.DaylightDate.wDay >
      5)))) then
    begin
...
function TSuperTimeZone.DayLightCompareDate(const date,
  compareDate: PSystemTime): Integer;
var
...
begin
  ...
  *)
  ...
  begin
    ...
    if (limit_day > MonthDays[(date^.wMonth = 2) and IsLeapYear(date^.wYear)
      ][date^.wMonth]) then
      Dec(limit_day, 7);
  end
  else
  ...
  dayinsecs := ((date^.wDay * 24 + date^.wHour) * 60 + date^.wMinute) * 60 +
    date^.wSecond;
...
class function TSuperTimeZone.ParseISO8601Date(const ISO8601Date: SOString;
  ...
  function get(var v: Word; c: SOChar): Boolean; {$IFDEF HAVE_INLINE} inline;
  {$ENDIF}
  begin

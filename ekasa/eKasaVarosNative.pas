...
const
  rs_connection_error =
    '{"errorCode":500,"error":"Nepodarilo sa spojenie s Varos. Skontrolujte ju!"}';

resourcestring
  receipt_header = 'N·zov tovaru' + #$D#$A +
    'Mnoûstvo  MJ  cena/MJ   DPH  cena celkom';
...
begin
  err := C_OK;
  if not varosOpen then
  begin
    case fSettings.i['ekasa.connectionTyp'] of
      ord(connectionRS232), ord(connectionUSB):
        err := fVaros._ConnectCom(fSettings.i['ekasa.comPort']); // COM/USB
      ord(connectionTCP):
        err := fVaros._ConnectTcp(fSettings.S['ekasa.hostAddress']); // TCP
    end;
    if (err <> C_OK) then
      exit;
    varosOpen := true;
...
procedure internalClose();
begin
  if (varosOpen = false) then
    exit;
  try
    case fSettings.i['ekasa.connectionTyp'] of
      ord(connectionRS232), ord(connectionUSB):
        fVaros._DisconectCom();
      ord(connectionTCP):
        fVaros._DisconnectTcp();
    end;
  finally
    varosOpen := false;
  end;
...
function totalTaxId(tax, count: currency; m_unit, tax0: string;
...
begin
  ...
  begin // zaporne danove hladiny
    ...
    begin
      ...
      else if (m_unit = '_OZ') then
      begin
        result := result + 'CN';
        if (tax_level = 3) then
          result := result + tax0;
      end
      else if (m_unit = '_VP') then
      begin
        result := result + 'DN';
        if (tax_level = 3) then
          result := result + tax0;
...
begin
  result := 'N';
  if (vatRate = 'OOD') then
    result := 'N' // osloboden· od dane
  else if (vatRate = 'PDP') then
    result := 'P' // prenesenie daÚovej povinnosti
  else if (vatRate = 'CK') then
    result := 'K' // cestovnÈ kancel·rie
  else if (vatRate = 'PT') then
    result := 'T' // pouûit˝ tovar
  else if (vatRate = 'UD') then
    result := 'U' // umeleckÈ diela
  else if (vatRate = 'ZPS') then
    result := 'Z'; // zberateæskÈ predmety a staroûitnosti
end;
...
begin
  ...
  try
    ...
    begin
      init_buffer_str();
      err := fVaros.STATE_s_x73(buffer_str);
      ...
      o.S['arr[]'] := '   stav spÌnaËa z·suvky: ' +
        oo.S['printerStatus.drawerPin'];
      o.S['arr[]'] := '   online stav tlaËiarne: ' +
        oo.S['printerStatus.online'];
      o.S['arr[]'] := '   Ëakanie na opravu: ' +
        oo.S['printerStatus.waitForRecowery'];
      o.S['arr[]'] := '   posun papiera: ' + oo.S['printerStatus.paperFeed'];
      o.S['arr[]'] := 'offlineStatus:';
      o.S['arr[]'] := '   otvoren˝ kryt tlaËiarne: ' +
        oo.S['offlineStatus.coverOpen'];
      ...
      o.S['arr[]'] := 'errorStatus:';
      o.S['arr[]'] := '   odstr·niteæn· chyba: ' +
        oo.S['errorStatus.recoverable'];
      o.S['arr[]'] := '   chyba rezaËa: ' + oo.S['errorStatus.autocutter'];
      o.S['arr[]'] := '   neodstr·niteæn· chyba: ' +
        oo.S['errorStatus.unrecoverable'];
      o.S['arr[]'] := '   odstr·niteæn· chyba: ' +
        oo.S['errorStatus.autorecoverable'];
      o.S['arr[]'] := 'sensorStatus:';
      o.S['arr[]'] := '   blÌûi sa koniec papiera: ' +
        oo.S['sensorStatus.paperNearEnd'];
    ...
    begin
      ...
      try
        ...
        begin
          ...
          bRefund := checkRefund();
          S := ESC + 'b';
          S := S + ifThen((reqObj.S['ReceiptData.Custom.Email'] = ''), '^t',
            '^e' + reqObj.S['ReceiptData.Custom.Email'] + '^k');
          if (reqObj.B['ReceiptData.Paragon']) then
          begin
            // cislo paragonu je nutne doplnit na dlzku 5 znakov zlava nulami
            paragonNumber :=
              RightStr(StringOfChar('0', 5) + reqObj.S
              ['ReceiptData.ParagonNumber'], 5);
            S := S + '^P' + getVarosNativeDateTime
              (reqObj.S['ReceiptData.IssueDate']) + paragonNumber + '^k';
          end;
          ...
          begin
            ...
            begin // zlava
              ...
              S := S + 'v DPH ' + totalTaxId(get_vatVal(item.S['VatRate']), -1,
                item.S['Custom.Unit'], get_vat0Id(item.S['VatRate']), true) +
            ...
            begin
              ...
              three_nnx := totalTaxId(get_vatVal(item.S['VatRate']),
                item.C['Quantity'], item.S['Custom.Unit'],
                get_vat0Id(item.S['VatRate']), false);
          ...
          sl.Add(ESC + 'k' + ifThen(cashRoundSupport = 1,
            value_x(totalSum + roundSum, 2), value_x(totalSum, 2)));
          ...
          // vydavok
          if ((reqObj.C['ReceiptData.Custom.CashReturn'] <> 0) and
            (bRefund = false)) then
          begin
            sl.Add('V˝davok:');
            sl.Add(ESC + 'P1' +
        ...
        begin
          ...
          if (totalSum > 0) then
            sl.Add(RightStr(StringOfChar(' ', 36) + ESC + '0NNN' +
              value_x(totalSum, 2) + ' EUR', 40))
          else
            sl.Add(RightStr(StringOfChar(' ', 36) + ESC + '9ANN' +
              value_x(totalSum, 2) + ' EUR', 40));
          ...
          if not Empty(reqObj.S['ReceiptData.Custom.Cashier']) then
            sl.Add(LeftStr('Meno: ' + reqObj.S
              ['ReceiptData.Custom.Cashier'], 40));
          if not Empty(reqObj.S['ReceiptData.Custom.Purpose']) then
            sl.Add(LeftStr('⁄Ëel: ' + reqObj.S
              ['ReceiptData.Custom.Purpose'], 40));
        ...
        begin
          ...
          // variabilna paticka
          if not Empty(reqObj.S['ReceiptData.Custom.Cashier']) then
            sl.Add(Copy('Meno: ' + reqObj.S
              ['ReceiptData.Custom.Cashier'], 0, 40));
          if not Empty(reqObj.S['ReceiptData.Custom.Purpose']) then
            sl.Add(Copy('⁄Ëel: ' + reqObj.S
              ['ReceiptData.Custom.Purpose'], 0, 40));
  ...
  try
    ...
    begin
      ...
      try
        sl.Clear;
        sl.Add(ESC + 'G');
        sl.Add('^AS' + reqObj.S['PhysicalAddress.StreetName'] + '^k');
        // mandatory
        sl.Add('^AM' + reqObj.S['PhysicalAddress.Municipality'] + '^k');
        // mandatory
        if (reqObj.S['PhysicalAddress.BuildingNumber'] <> '') then
          sl.Add('^AB' + reqObj.S['PhysicalAddress.BuildingNumber'] + '^k');
        // optional
        if (reqObj.S['PhysicalAddress.PostalCode'] <> '') then
          sl.Add('^AP' + reqObj.S['PhysicalAddress.PostalCode'] + '^k');
        // optional
        if (reqObj.S['PhysicalAddress.PropertyRegistrationNumber'] <> '') then
          sl.Add('^AN' + reqObj.S
            ['PhysicalAddress.PropertyRegistrationNumber'] + '^k');
        sl.Add(ESC + 'e');
...
begin
  ...
  case action of
    actState:
      result := varosNativeState;
    actSettingsGet:
      result := '{"errorCode":500,"error":"unsupported request"}';
    actSettingsPost:
      result := '{"errorCode":500,"error":"unsupported request"}';
    actCopyLast:
      result := varosNativeCopyLast;
    actCopyByUuid:
      result := '{"errorCode":500,"error":"unsupported request"}';
    actCopyById:
      result := '{"errorCode":500,"error":"unsupported request"}';
    actReceipt:
      result := varosNativeRecipt;
    actReceiptStateId:
      result := '{"errorCode":500,"error":"unsupported request"}';
    actLocationGps:
      result := varosNativeLocationGps;
    actLocationAddress:
      result := varosNativeLocationAddress;
    actLocationOther:
      result := varosNativeLocationOther;
    actReport:
      result := varosNativeReport;
    actUnsent:
      result := '{"errorCode":500,"error":"unsupported request"}';
    actSendunsent:
      result := varosNativeSendunsent;
    actPrncdkick:
      result := varosNativePrncdkick;
    actPrnfreeprint:
      result := '{"errorCode":500,"error":"unsupported request"}';
    actSenderror:
      result := '{"errorCode":500,"error":"unsupported request"}';

...
const
  rs_connection_error =
    '{"error":"Nepodarilo sa spojenie s eKASA 8000. Skontrolujte ju!"}';
  rs_connection_timeout =
    '{"error":"Prerušenie spojenia, resp. vypršal èas na odpoveï!"}';
...
begin
    ...
    try
      ...
      repeat
        ...
        try
          ...
          if (fHttp.ResponseCode in [200, 201]) then
          begin
            repeatReq := false;
            if fSettings.B['ekasa.withLog'] then
              addLog(Format('Ekasa POST Request: url-%s, data-%s',
                [url, data]));
          end;
        except
          on E: EIdHTTPProtocolException do
          begin
            if (fHttp.ResponseCode = 500) then
            begin
  ...
  if fSettings.B['ekasa.withLog'] then
    addLog(Format('Ekasa POST Response: result-%s', [result]));
end;
...
{$ENDIF}
    ...
    try
      ...
      repeat
        ...
        try
          ...
          result := getContent(stream.DataString,
          ...
          begin
            repeatReq := false;
            if fSettings.B['ekasa.withLog'] then
              addLog(Format('Ekasa GET Request: url-%s', [url]));
          end;
        except
          on E: EIdHTTPProtocolException do
          begin
  ...
  if fSettings.B['ekasa.withLog'] then
    addLog(Format('Ekasa GET Response: result-%s', [result]));
end;
